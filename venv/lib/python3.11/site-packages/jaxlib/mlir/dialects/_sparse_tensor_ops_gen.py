...
class PackOp(_ods_ir.OpView):
  ...
  def __init__(self, result, values, coordinates, *, batched_lvls=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    _ods_context = _ods_get_default_loc_context(loc)
    if batched_lvls is not None: attributes["batched_lvls"] = (batched_lvls if (
        issubclass(type(batched_lvls), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('IndexAttr')) else
          _ods_ir.AttrBuilder.get('IndexAttr')(batched_lvls, context=_ods_context))
    results.append(result)
    _ods_successors = None
    super().__init__(self.build_generic(
      attributes=attributes, results=results, operands=operands,
  ...
  @builtins.property
  def batched_lvls(self):
    if "batched_lvls" not in self.operation.attributes:
      return None
    return _ods_ir.IntegerAttr(self.operation.attributes["batched_lvls"])

  @batched_lvls.setter
  def batched_lvls(self, value):
    if value is not None:
      self.operation.attributes["batched_lvls"] = value
    elif "batched_lvls" in self.operation.attributes:
      del self.operation.attributes["batched_lvls"]

  @batched_lvls.deleter
  def batched_lvls(self):
    del self.operation.attributes["batched_lvls"]
...
class UnpackOp(_ods_ir.OpView):
  ...
  def __init__(self, values, coordinates, nse, tensor, *, batched_lvls=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
